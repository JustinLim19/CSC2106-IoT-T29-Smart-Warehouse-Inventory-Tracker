...
static BLEAddress *pServerAddresses[3];
...
bool isAllNodesDiscovered = false;
...
void setup() {
  ...
  for (int i = 0; i < foundDevices.getCount(); i++) {
    if (!isAllNodesDiscovered) {
      BLEAdvertisedDevice res = foundDevices.getDevice(i);
      if (
        res.getName() == cornerNodes[0] ||
        res.getName() == cornerNodes[1] ||
        res.getName() == cornerNodes[2]
      ) {
        Serial.print(res.getName().c_str());
        Serial.print(" @ ");
        Serial.println(res.getAddress().toString().c_str());
        pServerAddresses[currNodeIndex] = new BLEAddress(res.getAddress());
        currNodeIndex++;
      }
      if (currNodeIndex == 3) {
        currNodeIndex = 0;
        isAllNodesDiscovered = true;
        doConnect = true;
      }
    }    
  }
